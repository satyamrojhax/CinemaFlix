...
const TvShowHoverCard: React.FC<{ id: number; fullWidth?: boolean }> = ({ id, fullWidth }) => {
  ...
  const firstReleaseYear = tv.first_air_date ? new Date(tv.first_air_date).getFullYear() : undefined;
  const lastReleaseYear = tv.last_air_date ? new Date(tv.last_air_date).getFullYear() : undefined;
